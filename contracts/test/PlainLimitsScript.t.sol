...
import {PlainLimits} from "../script/PlainLimits.s.sol";
...
contract PlainLimitsHarness is PlainLimits {
    function rewriteAll(FuguRegistry registry, address sender) external {
        _rewriteAll(registry, sender);
    }

    function rewriteField(string memory uri, string memory value) external pure returns (string memory) {
        return _rewriteField(uri, value);
    }
...
contract PlainLimitsScriptTest is Test {
    PlainLimitsHarness harness;
    ...
    function setUp() public {
        harness = new PlainLimitsHarness();
    ...
    function _liveShapedMetadata(uint256 i) internal pure returns (string memory) {
        string memory json = string.concat(
            '{"name":"Agent ',
            vm.toString(i),
            '","summary":"Keeps your holdings at the mix you chose.',
            '","limits":"MCP/A2A tool; updateListing has no setter for agentWallet; see STATUS.md A4, A9, B1.',
    ...
    function test_rewritesEveryLimitsAndLeavesTheEvidenceAlone() public {
        ...
        for (uint256 id = 1; id <= 4; ++id) {
            ...
            assertEq(vm.indexOf(json, "updateListing has no setter"), type(uint256).max, "jargon still on the card");
            // ...and the fields a checker reads are byte-identical.
            assertTrue(
                vm.indexOf(json, "Keeps your holdings at the mix you chose.") != type(uint256).max,
                "summary lost"
            );
    ...
    function test_revertsWhenTheListingHoldsADifferentAgent() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(PlainLimits.WrongListing.selector, 1, 8004, 9999));
        harness.rewriteAll(other, owner);
    }

    function test_revertsWhenTheBroadcasterIsNotTheListingOwner() public {
        address stranger = address(0xBAD);
        vm.expectRevert(abi.encodeWithSelector(PlainLimits.NotListingOwner.selector, owner, stranger));
    ...
    function test_revertsIfTheMetadataWouldNotActuallyChange() public {
        harness.rewriteAll(registry, owner);
        vm.expectRevert(abi.encodeWithSelector(PlainLimits.MetadataUnchanged.selector, uint256(1)));
        harness.rewriteAll(registry, owner);
    }
}
