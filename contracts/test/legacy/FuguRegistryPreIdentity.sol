...
//
// This is `src/FuguRegistry.sol` exactly as it stood BEFORE ERC-8004 ownership checks
// and `rebindAgentId` were added: the implementation deployed at
// `0x13836c1bc0d32def6b6c0ec2acf120c2f342f3eb` on BSC testnet, behind the live proxy
// `0xb2f36070E6eae3353E8e755172B477DF213ae248`, which holds listings 1..9 with the
// placeholder ids 8004..8012.
//
// It exists so `test/IdentityUpgradeSafety.t.sol` can register listings the way the
// live proxy registered them, upgrade to the current implementation, and read them
// back — the same reasoning as `FuguRegistryLegacy.sol`, one upgrade later.
//
// Regenerated from git with (run from the repo root):
//
//     git show 88d330d:contracts/src/FuguRegistry.sol
//
// The only edits applied to that source: the `IFuguRegistry` import and the interface
// in the inheritance list were dropped (an interface carries no storage, so no slot
// moves, and the interface has since grown functions this version does not have), and
// `FuguRegistry` -> `FuguRegistryPreIdentity`. It shares `Category` and `Listing`
// with `src/` on purpose: this upgrade does not touch either type, and
// `CategoryUpgradeSafety.t.sol` is what guards them.
// ---------------------------------------------------------------------------------
...
contract FuguRegistryPreIdentity is Initializable, UUPSUpgradeable, OwnableUpgradeable {
