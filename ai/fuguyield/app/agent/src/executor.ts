 ...
 * commerce path masks a fault as "seller operation failed; retry later" because a buyer must
 * not learn anything from a signing error — but a REJECTED ADVISORY REQUEST is the caller's
 * own malformed position, and hiding the reason would leave them retrying the same broken
 * payload forever. `yieldAdvisorySkill` therefore never throws: it returns either the advice
 * or a rejection carrying the engine's own message.
 *
...
function advisoryDispatch(
  ...
  if (skill === YIELD_ADVISORY_SKILL) return yieldAdvisorySkill(data);
  if (skill === YIELD_BREAKEVEN_SKILL) return yieldBreakevenSkill(data);
  return null;
}
...
export class SellerAgentExecutor extends SellerCore implements AgentExecutor {
   ...
   * otherwise answer "skills: []" while happily answering `yield_advisory`.
   */
