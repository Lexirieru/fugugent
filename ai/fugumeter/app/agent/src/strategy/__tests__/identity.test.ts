...
describe("who this agent is on the blockchain", () => {
  it("uses the wallet already written into its listing, and no other", () => {
    ...
    expect(AGENT_WALLET).toBe("0x95c3c77e3B7d3873BcF6b9F4b12f47775e7312c8");
  });

  it("is listed under the id and category the rest of the repo agrees on", () => {
    expect(LISTING_ID).toBe(8);
    expect(CATEGORY_INDEX).toBe(7);
    expect(CATEGORY_NAME).toBe("STREAMING");
    expect(AGENT_LABEL).toBe("Fugu Meter");
  });
