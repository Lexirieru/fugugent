...
import {
  ...
  purchaseAdvisoryToolShape,
  purchasePolicyRequestSchema,
  purchasePolicySkill,
} from "./purchase.js";
    ...
    cachedRunLlm = async (p: string) => {
      const result = await generateText({
        ...
        system:
          ...
          "must come from the `purchase_advisory` and `purchase_policy` tools, which run code " +
          "that always answers the same way for the same input. Quote what they return; never " +
          "invent, adjust, average or extend a figure, and never say a request was paid for " +
          "when the tools did not say so. Deciding to pay and having paid are different " +
          "things, and only a transaction proves the second. " +
...
export function buildMcpServer(
  ...
  // ── The purchase tools (the deliverable) ───────────────────────────────────
  // Registered whatever else is configured, unlike the commerce tools: the decision needs
  // no payment rail, no wallet and no network. It is a function of its argument.
  //
  ...
  const purchaseAnnotations = { readOnlyHint: true, openWorldHint: false };

  server.registerTool(
    PURCHASE_ADVISORY_SKILL,
    {
      description:
        "DECISION ONLY. Given a seller's reply that demands payment, decide whether to pay it " +
        "and with which of the ways it offered. Checks the chain, the token, who gets paid, " +
        "the price against a per-call cap, what is left of the spending window, and how long " +
        "the authorisation would stay usable. Answers PAY, REFUSED or WINDOW_EXHAUSTED with " +
        "every option put aside and the rule that did it. " +
        "THIS TOOL DOES NOT PAY. It signs nothing and sends no transaction.",
      inputSchema: purchaseAdvisoryToolShape,
      annotations: purchaseAnnotations,
    },
    async (args) => toolResult(purchaseAdvisorySkill(args)),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "The limits this agent buys within: the chain, the tokens, the ways it signs, the most " +
        "per call, the most per window, and the longest an authorisation stays usable. Each " +
        "comes with the reason it is the value it is.",
      inputSchema: purchasePolicyRequestSchema.shape,
      annotations: purchaseAnnotations,
    },
    async (args) => toolResult(purchasePolicySkill(args)),
  );
