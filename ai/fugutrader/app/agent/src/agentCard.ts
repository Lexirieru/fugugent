...
import { PURCHASE_ADVISORY_SKILL, PURCHASE_POLICY_SKILL } from "./purchase.js";
 ...
 * BEFORE paying. It decides and it explains. Signing is a separate step with its own
 * evidence, and that evidence is a transaction.
 */
const PURCHASE_ADVISORY: AgentSkill = {
  id: PURCHASE_ADVISORY_SKILL,
  name: "Decide whether one paid request is worth paying for",
  description:
    'Send a data part {"skill": "' + PURCHASE_ADVISORY_SKILL + '", "demand": {...}} where ' +
    "`demand` is the seller's reply body exactly as it arrived, the one carrying its list of " +
    "ways to be paid. The engine checks the chain, the token, who would be paid, the price " +
    "against a per-call cap, what is left of the spending window, and how long the " +
    "authorisation would stay usable, then answers PAY, REFUSED or WINDOW_EXHAUSTED with " +
    "every option it put aside and the rule that did it. " +
    "THIS SKILL DOES NOT PAY. It signs nothing and sends no transaction. A demand it cannot " +
    "read comes back as a refusal with the reason, never as a guess.",
  tags: ["commerce", "pay-per-call", "decision", "no-payment", "bnb-chain"],
  inputModes: ["application/json"],
  outputModes: ["application/json"],
};

const PURCHASE_POLICY: AgentSkill = {
  id: PURCHASE_POLICY_SKILL,
  name: "The limits this agent buys within",
  description:
    'Send {"skill": "' + PURCHASE_POLICY_SKILL + '"} to read the chain this agent pays on, ' +
    "the tokens it pays with, the ways it will sign, the most it pays for one call, the most " +
    "it pays across a window, and the longest it will leave an authorisation usable. Each " +
    "comes with the reason it is the value it is. Nothing is paid by asking.",
  tags: ["commerce", "limits", "decision", "no-payment"],
  inputModes: ["application/json"],
  outputModes: ["application/json"],
};
...
export function buildAgentCard(
  ...
  return {
    name,
    description:
      `Fugu Trader (${name}). It buys one call at a time: it reads a seller's demand for ` +
      "payment, decides with rules that are written down and tested, and can sign one " +
      "authorisation for that exact amount, valid for a few minutes and no longer. Neither " +
      "side ever holds the other's key. Deciding and paying are separate steps: this card " +
      "describes the deciding, and a payment is only ever proven by a transaction. Commerce: " +
      "negotiate and notify_funded.",
    ...
    // The purchase skills stand first and are never gated: they are what the agent is for,
