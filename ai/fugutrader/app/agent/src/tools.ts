...
/**
 * The purchase tools, which are the reason this agent is worth renting.
 *
 * They read a seller's demand for payment, run the deterministic engine in
 * `src/strategy/`, and return the decision together with the reasoning behind it. The
 * model may call them and read the result out loud. It may not choose an option, move a
 * cap, or recompute an amount.
 *
 * They DECIDE. They do not sign anything and they do not pay. The descriptions say so,
 * because a model that believes a tool paid a seller will tell a person the seller was
 * paid.
 */
export const STRATEGY_TOOLS: ToolSet = {
  purchase_advisory: tool({
    description:
      "DECISION ONLY. Given a seller's reply that demands payment, decides whether to pay it " +
      "and with which of the ways it offered. Checks the chain, the token, who gets paid, the " +
      "price against a per-call cap, what is left of the spending window, and how long the " +
      "authorisation would stay usable. The answer is PAY, REFUSED or WINDOW_EXHAUSTED, with " +
      "every option that was put aside and the rule that did it. " +
    ...
    inputSchema: purchaseAdvisoryInputSchema,
    execute: async (input) => advisePurchase(input).payload,
  }),
  purchase_policy: tool({
    description:
      "The limits this agent buys within: the chain it pays on, the tokens it pays with, the " +
      "ways it will sign, the most it pays for one call, the most it pays across a window, and " +
      "the longest it will leave an authorisation usable. Each comes with the reason it is the " +
      "value it is. Read this rather than guessing why a purchase was or was not made.",
    inputSchema: purchasePolicyRequestSchema,
    execute: async (input) => purchasePolicySkill(input),
  }),
};

export const LLM_READ_TOOLS: ToolSet = {
  // --- The purchase decision (pure code, never the model's judgement) ---
  ...STRATEGY_TOOLS,
