 ...
 * buyer must learn nothing from a signing error. A REFUSED PURCHASE REQUEST is the
 * opposite case: it is the caller's own unreadable demand, and hiding the reason would
 * leave them sending the same broken payload forever. `purchaseAdvisorySkill` therefore
 * never throws; it returns either the decision or a refusal carrying the engine's message.
 *
 * Neither skill touches the wallet, the chain, or the model.
 */
function purchaseDispatch(
  skill: unknown,
  data: Record<string, unknown>,
): Record<string, unknown> | null {
  if (skill === PURCHASE_ADVISORY_SKILL) return purchaseAdvisorySkill(data);
  if (skill === PURCHASE_POLICY_SKILL) return purchasePolicySkill(data);
  return null;
}
...
export class SellerAgentExecutor extends SellerCore implements AgentExecutor {
   ...
   */
  /**
   * The purchase skills are always served, so they always belong in the list a caller gets
   * back after sending an unknown skill. Without this override a seller with no payment
   * rail configured would answer "skills: []" while happily answering `purchase_advisory`.
   */
  override skills(): string[] {
    return [...PURCHASE_SKILL_IDS, ...super.skills()];
  }
  ...
  async dispatch(
    ...
    const purchase = purchaseDispatch(skill, data);
  ...
  execute = async (
    ...
    const purchase = purchaseDispatch(skill, data);
    if (purchase !== null) {
      reply(eventBus, context, purchase);
      return;
    }
