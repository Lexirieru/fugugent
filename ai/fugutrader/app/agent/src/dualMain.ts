...
import { FUGU_X402_PATH, buildX402Seller, x402SellerRouter } from "./x402/route.js";
...
export async function buildDualApp(): Promise<{
  ...
  // The selling half, when it is switched on. It is built BEFORE any listener opens and
  // its configuration faults are NOT caught: a seller that demands payment and then
  // rejects every payment that arrives is worse than one that is switched off, and a
  // buyer cannot tell the two apart.
  const fuguSeller = buildX402Seller(process.env);
  if (fuguSeller !== null) {
    app.use(x402SellerRouter(fuguSeller.merchant));
    console.log(
      `[fugu-trader] selling at POST ${FUGU_X402_PATH}: ${fuguSeller.config.price} units of ` +
        `${fuguSeller.config.rails[0]?.token.symbol ?? "the payment token"} per call, paid to ` +
        `${fuguSeller.config.payTo}`,
    );
  }
