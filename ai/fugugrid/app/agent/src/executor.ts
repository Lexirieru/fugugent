 ...
 * caller's own malformed grid, and hiding the reason would leave them retrying the same
 * broken payload forever. `gridAdvisorySkill` therefore never throws: it returns either the
 * advice or a rejection carrying the engine's own message.
 *
...
function advisoryDispatch(
  ...
  if (skill === GRID_ADVISORY_SKILL) return gridAdvisorySkill(data);
  if (skill === GRID_FEASIBILITY_SKILL) return gridFeasibilitySkill(data);
  return null;
}
...
export class SellerAgentExecutor extends SellerCore implements AgentExecutor {
   ...
   * otherwise answer "skills: []" while happily answering `grid_advisory`.
   */
