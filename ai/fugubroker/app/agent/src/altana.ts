 ...
 * "send this batch" function. Used by the operational scripts (granting the key,
 * probing the boundary) AND by the agent runtime. No strategy logic here.
 *
 * It lives in `src/` rather than `scripts/` because the running agent needs it: a copy of
 * this file inside the runtime would be a second definition of "how the key sends a
 * payment", and the first one to drift would be discovered by a transaction, not by a test.
 * `src/strategy/` still contains none of it — the strategy modules must not depend on a
 * third-party SDK, which is exactly why `createSessionHire` takes `sendCalls` as an argument.
 *
 ...
 * `studio-runtime/dist/chunk-ZZZFODYE.js:76`. The Broker key here specifically MUST
 * have a different allowlist (subscribe + approve, nothing more), so it is loaded via
...
/** `.../ai/fugubroker/app/agent` — the project root `bag` uses. */
export const AGENT_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "..");

/** `.../ai/fugubroker` — the workspace `.studio/` lives in. */
...
/**
 * The Broker's own bounded key file. SEPARATE from `altana-session.json` (the commercial
 * ERC-8004/8183 session `bag dev` uses), so neither overwrites the other.
 */
export const BROKER_SESSION_FILE = path.join(KEYSTORE_DIR, "altana-session-broker.json");
...
/** Loads the Broker key from disk. The `signer` content is never touched here. */
export async function loadBrokerSession(file = BROKER_SESSION_FILE): Promise<AltanaSession> {
...
/**
 * An admin-mode provider from the encrypted keystore. Used ONLY for setup, which here means
 * granting the bounded key. It never pays for a rental.
 */
 ...
 * **returns ERC-20 allowances to zero at the end of the same userOp** — proven on the
 * Guardian task's first run (tx `0xf9570e1e...` carried two `Approval` events: 11.67 then 0), so a
 * a payment in the next transaction failed with `ERC20InsufficientAllowance`. That behavior is
 * in fact correct — an allowance from a leaked key must not outlive its transaction — and
 * the answer is to send `approve` and the payment as ONE atomic userOp.
 *
