...
/**
 * The hiring skills, over A2A.
 *
 * Handled BEFORE the commerce skills, and outside their try/catch, on purpose. The
 * commerce path turns any fault into "seller operation failed; retry later", because a
 * buyer must learn nothing from a signing error. A REJECTED HIRING REQUEST is the
 * opposite case: it is the caller's own malformed catalog, and hiding the reason would
 * leave them sending the same broken payload forever. `hireAdvisorySkill` therefore never
 * throws; it returns either the decision or a rejection carrying the engine's own message.
 *
 * Neither skill touches the wallet, the chain, or the model. Both are functions of the
 * data part and nothing else.
 */
function hiringDispatch(
  skill: unknown,
  data: Record<string, unknown>,
): Record<string, unknown> | null {
  if (skill === HIRE_ADVISORY_SKILL) return hireAdvisorySkill(data);
  if (skill === HIRE_POLICY_SKILL) return hirePolicySkill(data);
  return null;
}
...
export class SellerAgentExecutor extends SellerCore implements AgentExecutor {
   ...
   */
  /**
   * The hiring skills are always served, so they always belong in the list a caller gets
   * back after sending an unknown skill. Without this override a seller with no payment
   * rail configured would answer "skills: []" while happily answering `hire_advisory`.
   */
  override skills(): string[] {
    return [...HIRING_SKILL_IDS, ...super.skills()];
  }
  ...
  async dispatch(
    ...
    const hiring = hiringDispatch(skill, data);
