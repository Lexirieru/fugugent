    ...
    cachedRunLlm = async (p: string) => {
      const result = await generateText({
        ...
        system:
          ...
          "DECISIONS ABOUT MONEY ARE NOT YOURS TO MAKE. Every number, limit and choice " +
          "must come from the `hire_advisory` and `hire_policy` tools, which run code that " +
          "always answers the same way for the same input. Quote what they return; never " +
          "invent, adjust, average or extend a figure, and never name a listing they did not " +
          "choose. Deciding and paying are different things: describe the result as a choice " +
          "and its price, never as money that has moved, is moving, or is about to move. A " +
          "payment is only ever proven by a transaction. " +
...
export function buildMcpServer(
  ...
  // ── The hiring tools (the deliverable) ─────────────────────────────────────
  // Registered whatever else is configured, unlike the commerce tools: the decision
  // needs no payment rail, no wallet and no network. It is a function of its argument.
  //
  ...
  const hiringAnnotations = { readOnlyHint: true, openWorldHint: false };

  server.registerTool(
    HIRE_ADVISORY_SKILL,
    {
      description:
        "DECISION ONLY. Choose which agent in a catalog to rent, for how many blocks of time, " +
        "and at what cost, or refuse and say why. Send the catalog, the capability wanted, how " +
        "long the work takes and the spending limit; money is on the 8-decimal basis, so " +
        "100000000 is one dollar. The answer is HIRE, NO_MATCH, OVER_BUDGET, BLOCKED_BY_POLICY " +
        "or WINDOW_EXHAUSTED, with the listing chosen, the one it was chosen against, and the " +
        "rule that put every other listing aside. " +
        "THIS TOOL DOES NOT PAY. It signs nothing and sends no transaction. A malformed catalog " +
        "is refused with the reason, never answered with a guess.",
      inputSchema: hireAdvisoryToolShape,
      annotations: hiringAnnotations,
    },
    async (args) => toolResult(hireAdvisorySkill(args)),
  );
  ...
  server.registerTool(
    ...
    {
      description:
        "The limits this agent spends within: the ceiling for one rental, the ceiling across a " +
        "window, the most it pays for one block of time, the longest prepayment, the block " +
        "lengths it rents by, and how far a price may move between the decision and the " +
        "payment. Each comes with the reason it is the value it is. Read this rather than " +
        "guessing why the agent did or did not rent something.",
      inputSchema: hirePolicyRequestSchema.shape,
      annotations: hiringAnnotations,
    },
    async (args) => toolResult(hirePolicySkill(args)),
  );
