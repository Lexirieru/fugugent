...
export const BLOAT: Record<BloatLevel, BloatLevelSpec> = {
  ...
  3: {
    level: 3,
    name: "Strained",
    state: "Inside the band where the money stops being able to look after itself.",
    ring: "dashed ring",
    color: "var(--risk-3)",
    motionClass: null,
  },
...
export const GUARDIAN_ACTIONS: readonly GuardianActionSpec[] = [
  ...
  },
  {
    ...
    level: 3,
    does: "Pays back debt up to the safe target, no further, and inside a limit per action and per day.",
    spendsMoney: true,
  },
  {
    action: "DELEVERAGE",
    level: 4,
    does: "A larger, more urgent payment toward the same target. It reduces the debt. It does not sell the collateral.",
    spendsMoney: true,
  },
  {
    action: "EMERGENCY",
    level: 5,
    does: "Pays back at the limit immediately. From here anyone may force the loan closed and sell the collateral, so there is nothing gentler left to try.",
    spendsMoney: true,
  },
];
...
export function riskAriaLabel(agentName: string, reading: RiskReading | null): string {
  if (!reading) {
    return `${agentName}. No fresh risk reading, so the fish is drawn hollow rather than guessed.`;
  }
